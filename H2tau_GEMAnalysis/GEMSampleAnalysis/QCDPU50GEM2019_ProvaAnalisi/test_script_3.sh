...
set files_per_job = 10

set filenum = 0
# set jobnum = 0

rm -f fileinp_$filenum
rm -f histograms_MC_QCDPU50GEM2019$filenum.C
rm -f jobs_submit

set count = 0
foreach line ("`cat QCD_Pt-15to3000_Tune4C_Flat_14TeV_GEM2019Upg14DR-PU50bx25_GEM.txt`")
 @ count = $count + 1
...
#	echo $count
	if ($count % $files_per_job == 0) then
	 rm -f histograms_MC_QCDPU50GEM2019$filenum.C
         ...
         rm -f fileinp_$filenum
         sed s/histo_file_QCDPU50GEM2019XX/histo_file_QCDPU50GEM2019$filenum/ jobs2_$filenum >> histograms_MC_QCDPU50GEM2019$filenum.C
         rm -f jobs2_$filenum
