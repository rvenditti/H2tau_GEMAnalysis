...
void VHAnalyser_Jan13::Loop()
{
       ...
       if(WplusGOODmuon){
           for(Int_t r=0; r<nGenParticle; r++){
               ...
               if(TMath::Abs(GenParticle_pdgId[r]) == 13 && GenParticle_status[r] == 1 && GenParticle_motherIndex[r] == rigaMotherW){
                   ...
                   muW.SetPtEtaPhiE(GenParticle_pt[rigaMu], GenParticle_eta[rigaMu], GenParticle_phi[rigaMu], GenParticle_energy[rigaMu]);
		   cout<<jentry<<"pdgId  "<<GenParticle_pdgId[r]<<" mother "<<GenParticle_motherIndex[r]<<" pt  "<<GenParticle_pt[r]<<endl;
 ...
 MET.SetPtEtaPhiM(mvaMET_met[0], 0., mvaMET_metphi[0], 0.);
       ...
       std::vector<int>        indexMuonPtEndcap,indexMuonPtBarrel;
       //std::cout<<" nMuon "<<nMuon<<std::endl;
       bool ptCut1 =false;  bool ptCut2 =false; bool ptCut=false;
       for (Int_t i=0 ; i<nMuon ; i++){
	 mu_tmp.SetPtEtaPhiE(Muon_pt[i], Muon_eta[i], Muon_phi[i], Muon_energy[i]);
	 if(muW.DeltaR(mu_tmp)<0.03){
	   cout<<jentry<<"  muW= "<<muW.Pt()<<"   muReco="<<mu_tmp.Pt()<<endl;
	   ...
	   if(TMath::Abs(Muon_eta[i]) < 1.5) {
	     ...
	     hPtRes_Barrel->Fill(muW.Pt()-mu_tmp.Pt());
	   }

	   ///Muon Impact Parameter in High Eta
	   if(  ( (Muon_eta[i] >=  1.5) && (Muon_eta[i] <  2.1)) ||   (Muon_eta[i] <=  (-1.6)) && (Muon_eta[i] >  -(2.1))   ) {
	     hDB_HE->Fill(Muon_dB[i]);  
	     hDBSig_HE->Fill(Muon_dB[i]/Muon_edB[i]);
	     hChiSq_HE->Fill(Muon_globalChi2[i]);
	     hPtRes_HE->Fill(muW.Pt()-mu_tmp.Pt());
	 ...
	 if( TMath::Abs(Muon_eta[i])<1.5 ){
	   if(Muon_pt[i]>24){ indexMuonPtBarrel.push_back(i);
	     ptCut1=true;}
	 }
	 if( (TMath::Abs(Muon_eta[i])>1.5) && (TMath::Abs(Muon_eta[i])<2.1)){
	   if(Muon_pt[i]>10){ indexMuonPtEndcap.push_back(i);
	     ptCut2=true;}
	 }
	 ptCut =( ptCut1 || ptCut2);
	 if(ptCut==false) continue;
