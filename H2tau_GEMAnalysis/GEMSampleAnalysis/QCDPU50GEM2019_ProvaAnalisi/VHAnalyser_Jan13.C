...
void VHAnalyser_Jan13::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<100000;jentry++) {
	 ...
	 if(TMath::Abs(GenParticle_pdgId[r])==13) {
	   ...
	   IndexGenMu.push_back(r);
	   
	 }
	 
       }
       
       //      if( !(IndexGenMu.size()>0)) continue;
       ...
       TLorentzVector muGen; muGen.SetPtEtaPhiE(1,0,0,0);  
       for (Int_t i=0 ; i<nMuon ; i++){
	 muInitial.SetPtEtaPhiE(Muon_pt[i], Muon_eta[i], Muon_phi[i], Muon_energy[i]);
	 
	 for(Int_t r=0; r<IndexGenMu.size(); r++){
	   muGen.SetPtEtaPhiE(GenParticle_pt[IndexGenMu[r]], GenParticle_eta[IndexGenMu[r]], GenParticle_phi[IndexGenMu[r]], GenParticle_energy[IndexGenMu[r]]);
	   if( muGen.DeltaR(muInitial)<0.1 ){

	     //  cout<<jentry<<" muon reco pt pass match ="<<muInitial.Pt()<<endl;
	 ...
	 for(int k=0;k<IndexTauOS.size();k++){
		...
		*/
		...
		NoExtraMuon->Fill(1,puWeight);
		 ...
		 if(  ( TMath::Abs(Muon_eta[indexMuon[0]]) <2.1) && ( TMath::Abs(Muon_eta[indexMuon[0]]) >=1.6) ){
		   hMuonEtaFinal_HE->Fill(mu1.Eta());
		   hMuonPtFinal_HE->Fill(mu1.Pt());
		      }
		      
		 if(  ( TMath::Abs(Muon_eta[indexMuon[0]]) <1.6) ){
