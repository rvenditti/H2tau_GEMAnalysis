...
set count = 0
foreach line ("`cat wh.txt`")
 @ count = $count + 1
 
	echo "$line" >> fileinp_$filenum
#	echo $count
	if ($count % $files_per_job == 0) then
	 rm -f histograms_MC_WH_PU40CMS2012_ProvaAnalisi$filenum.C
         ...
         rm -f fileinp_$filenum
         sed s/histo_file_WH_PU40CMS2012_ProvaAnalisiXX/histo_file_WH_PU40CMS2012_ProvaAnalisi$filenum/ jobs2_$filenum >> histograms_MC_WH_PU40CMS2012_ProvaAnalisi$filenum.C
         rm -f jobs2_$filenum
