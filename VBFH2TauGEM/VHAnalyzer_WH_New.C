...
void VHAnalyzer_WH_New::Loop()
{
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	cout<<jentry<<" index="<<r<<" particle="<<GenParticle_pdgId[r]<<"  status="<<GenParticle_status[r]<<" mother index  "<<GenParticle_motherIndex[r]<<" pt  "<<GenParticle_pt[r]<<endl;        
      }
      */
	 ...
	 if(TMath::Abs(GenParticle_pdgId[r]) == 15 && GenParticle_status[r] == 3 && GenParticle_motherIndex[r] == rigaMotherH){
       ...
       Int_t rigaMu_tau=-2;
       TLorentzVector muGen_tau1, Tau1HadGen,Tau2HadGen, RecoTau ,muGen_tau2;
       muGen_tau1.SetPtEtaPhiE(1,0,0,0) ;
       muGen_tau2.SetPtEtaPhiE(1,0,0,0) ; 
       Tau1HadGen.SetPtEtaPhiM(1,0,0,0) ;   Tau2HadGen.SetPtEtaPhiM(1,0,0,0) ;   
	 ...
	 std::vector<int> Initial_barrel, Initial_endcap;
	 ...
	 for (Int_t i=0 ; i<nMuon ; i++){
	   ...
	   if( muGen_tau2.DeltaR(muInitial)<0.1 || muGen_tau1.DeltaR(muInitial)<0.1 ){
	     if(  ( TMath::Abs(muInitial.Eta())) < 1.6) {
	       Initial_barrel.push_back(1);  
	       //  hInitialBarrel->Fill(Muon_pt[i]);
	     }
	   if( ( (muInitial.Eta()  >=  1.6) && (muInitial.Eta() <  2.2)  ) ||   ( (muInitial.Eta() <=  (-1.6)) && (muInitial.Eta() >  -(2.2)) ) ) {
	     // hInitialHE->Fill(Muon_pt[i]);
	     Initial_endcap.push_back(1); 
	   }
	   }
	 }


	 if(Initial_barrel.size() !=0 ) hInitialBarrel->Fill(1);
	 if(Initial_endcap.size() !=0 ) hInitialEndcap->Fill(1);
	 ...
	 if(indexNJet.size()>2){OneJet=true;}
	 if(OneJet==false)continue;
	 hOneJet->Fill(1);
	 
	 TLorentzVector jet1,jet2,DiJet; jet1.SetPtEtaPhiE(1,0,0,0); jet2.SetPtEtaPhiE(1,0,0,0); 
	 jet1.SetPtEtaPhiE(Jet_pt[indexNJet[0]],Jet_eta[indexNJet[0]],Jet_phi[indexNJet[0]],Jet_energy[indexNJet[0]]);
	 jet2.SetPtEtaPhiE(Jet_pt[indexNJet[1]],Jet_eta[indexNJet[1]],Jet_phi[indexNJet[1]],Jet_energy[indexNJet[1]]);
	 DiJet = jet1+jet2;
	 double DiJetDeltaEta=TMath::Abs(jet1.Eta()-jet2.Eta());
	 double DiJetMass=DiJet.M();
	 
	 hDiJetDeltaEta->Fill(DiJetDeltaEta);
	 hDiJetMass->Fill(DiJetMass);
	 // cout<<jentry<<" delta Eta  "<<DiJetDeltaEta<<" mass  "<<DiJetMass<<endl;
	 
	 
	 if(!( (DiJetDeltaEta>3.5) &&  (DiJetMass>500) ) ) continue;
	...
	//std::cout<<" nMuon "<<nMuon<<std::endl;
	  ...
	  for (Int_t i=0 ; i<nMuon ; i++){
	    ...
	    if(mu_tmp.DeltaR(muGen_tau1)<0.1){
	      if(TMath::Abs(Muon_eta[i]) < 1.5) {
		hDB_Barrel->Fill(Muon_dB[i]); 
		hDBSig_Barrel->Fill(Muon_dB[i]/Muon_edB[i]); 
		hChiSq_Barrel->Fill(Muon_globalChi2[i]);  
		hDBerr_Barrel->Fill(Muon_edB[i]);
		hPtRes_Barrel->Fill((muGen_tau1.Pt()-mu_tmp.Pt())/muGen_tau1.Pt());
		hPtBarrel->Fill(mu_tmp.Pt());
	   }

	      ///Muon Impact Parameter in High Eta
	      if(  ( (Muon_eta[i] >=  1.5) && (Muon_eta[i] <  2.1)) ||   (Muon_eta[i] <=  (-1.5)) && (Muon_eta[i] >  -(2.1))   ) {
		hDB_HE->Fill(Muon_dB[i]);  
		hDBSig_HE->Fill(Muon_dB[i]/Muon_edB[i]);
		hChiSq_HE->Fill(Muon_globalChi2[i]);
		hPtRes_HE->Fill( (muGen_tau1.Pt()-mu_tmp.Pt())/muGen_tau1.Pt());
		hDBerr_HE->Fill(Muon_edB[i]);
		hPtHE->Fill(mu_tmp.Pt());
	      }
	      
	    }


	    if(mu_tmp.DeltaR(muGen_tau2)<0.1){
	      if(TMath::Abs(Muon_eta[i]) < 1.5) {
		hDB_Barrel->Fill(Muon_dB[i]); 
		hDBSig_Barrel->Fill(Muon_dB[i]/Muon_edB[i]); 
		hChiSq_Barrel->Fill(Muon_globalChi2[i]);  
		hDBerr_Barrel->Fill(Muon_edB[i]);
		hPtRes_Barrel->Fill((muGen_tau2.Pt()-mu_tmp.Pt())/muGen_tau2.Pt());
		hPtBarrel->Fill(mu_tmp.Pt());
	   }
		...
		hDBSig_HE->Fill(Muon_dB[i]/Muon_edB[i]);
		hChiSq_HE->Fill(Muon_globalChi2[i]);
		hPtRes_HE->Fill((muGen_tau2.Pt()-mu_tmp.Pt())/muGen_tau2.Pt());
		hDBerr_HE->Fill(Muon_edB[i]);
		hPtHE->Fill(mu_tmp.Pt());
	      }
	    ...
	    if(!(TMath::Abs(Muon_dB[i])<0.2))continue;
	    indexMuonTrackdB.push_back(i);
	    if(!(TMath::Abs(Muon_vz[i]-Vertex_z[IndexPV[0]])<0.5))continue;
	...
	//	std::cout<<jentry<<" after muon  "<<endl;
	  ...
	  for(Int_t j=0; j<nTau; j++){
	    ...
	    if(!(Tau_chargedIsoPtSum[j]<2.0)) continue;
	  ...
	  for (Int_t i=0 ; i<nMuon ; i++){
	    ...
	    if (!((Muon_isTrackerMuon[i]==1)&&(Muon_isGlobalMuonPromptTight[i]==1)&& (Muon_isPFMuon[i]==1)))continue;
	    if (!(Muon_pfRelIso[i] < 0.3)) continue;
	  ...
	  for(Int_t c=0; c<indexCoppiaSegnale.size(); c++){
		  tau_charge.SetPtEtaPhiE(Tau_pt[indexTau1coppiaSegnale[c]], Tau_eta[indexTau1coppiaSegnale[c]], Tau_phi[indexTau1coppiaSegnale[c]], Tau_energy[indexTau1coppiaSegnale[c]]);
		  mu_charge.SetPtEtaPhiE(Muon_pt[indexMuoncoppiaSegnale[c]], Muon_eta[indexMuoncoppiaSegnale[c]], Muon_phi[indexMuoncoppiaSegnale[c]], Muon_energy[indexMuoncoppiaSegnale[c]]);
		  if(!(tau1.DeltaR(tau2)>0.5)) continue;
		  
		  cout<<jentry<<"-----final mu  "<<Tau_pt[indexMuoncoppiaSegnale[c]]<<endl;
		  cout<<jentry<<"-----final tau "<<Tau_pt[indexTau1coppiaSegnale[c]]<<endl;
		  
		  MH = mu_charge + tau_charge;

		  // std::cout<<jentry<<" peso segnale  "<<pesoSegnale.at(c)<<" peso  "<<peso<<" puWeight  "<<puWeight<<endl;;
		  hTau1Pt[7]->Fill(tau_charge.Pt(), puWeight);
		  hTau1Eta[7]->Fill(tau_charge.Eta(),puWeight);
		  hMuon1Pt[7]->Fill(mu_charge.Pt(),puWeight);
		  hMuon1Eta[7]->Fill(mu_charge.Eta(),puWeight);
	  ...
	  if(  ( TMath::Abs(Muon_eta[indexMuon[0]]) <2.1) && ( TMath::Abs(Muon_eta[indexMuon[0]]) >=1.5) ){
	  ...
	  if(  TMath::Abs(Muon_eta[indexMuon[0]]) <2.1){
	    hMuonEtaFinal_Full->Fill(mu.Eta());
	    hMuonPtFinal_Full->Fill(mu.Pt());}
	  
	  if( ( TMath::Abs(Muon_eta[indexMuon[0]]) <2.1) && Muon_pt[indexMuon[0]]>20){
...
void VHAnalyzer_WH_New::bookHistograms() {
    if (!bookedHistos) {
		...
		hDiJetDeltaEta=new TH1F ("hDiJetDeltaEta","hDiJetDeltaEta",10,0,10);
		hDiJetMass=new TH1F ("hDiJetMass"," hDiJetMass",1000,0,1000);
		hVBFTag=new TH1F ("hVBFTag","hVBFTag",1000,0,1000);
	...
	hPtHE =  new TH1F("hPtHE","hPtHE",100,0,200);
	hPtBarrel =  new TH1F("hPtBarrel","hPtBarrel",100,0,200);

	hInitialBarrel =  new TH1F("hInitialBarrel","hInitialBarrel",100,0,200);
	hInitialEndcap =  new TH1F("hInitialEndcap","hInitialEndcap",100,0,200);
